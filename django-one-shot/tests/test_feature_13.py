...
class TestTodoItemUpdateView(TestCase):
    def setUp(self):
        self.client = Client()
        self.todolist = TodoList.objects.create(name="TodoList 1")
        self.todoitem = TodoItem.objects.create(
            task="TodoItem 1", due_date=timezone.now(), list=self.todolist
        )
        self.response = self.client.get(
            f"/todos/items/{self.todoitem.pk}/edit/"
        )
    ...
    def test_todo_item_create_resolves_to_todo_item_create(self):
        path = reverse("todo_item_update", args=[self.todoitem.pk])
        self.assertEqual(
            path,
            f"/todos/items/{self.todoitem.pk}/edit/",
            msg=f"Could not resolve path name 'todo_item_create' to '/todos/items/{self.todoitem.pk}/edit/",
        )
